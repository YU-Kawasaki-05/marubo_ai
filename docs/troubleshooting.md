...
* チャット入力中に突然「セッションが切れました」モーダルが表示される
* API が 401 を返し始める

### 原因

* Supabase Auth のアクセストークンが失効（デフォルト 1 時間）
* リフレッシュトークンが無効になった（長期間の非アクティブ）
* 別のタブでログアウトした

### 解決方法

1. `SessionExpiredModal` の再ログインボタンをクリックし、再認証する
2. モーダルが表示されない場合は、ブラウザの localStorage を確認し、`sb-` プレフィックスのキーが存在するか確認
3. `onAuthStateChange` で `SIGNED_OUT` イベントが正しく発火しているか DevTools で確認
...
* 「オフラインです」バナーが画面上部に表示される
* メッセージ送信が失敗する

### 原因

* Wi-Fi 切断、モバイルデータ不安定（学校の Wi-Fi 等で多発）
* Vercel のエッジサーバーへの接続不可

### 解決方法

1. ネットワーク接続を復旧させる。バナーはオンライン復帰時に自動で消える
2. 送信失敗したメッセージはエラーメッセージとともにインラインで表示されるので、再送信する
3. ストリーミング中断の場合は「回答が途中で中断されました」と表示される。再度メッセージを送信する

---

## Error Boundary によるフォールバック表示

### 症状

* 「エラーが発生しました。リロードしてください」というフォールバック UI が表示される

### 原因

* React コンポーネントのレンダリング中に未捕捉の例外が発生
* Markdown/KaTeX のレンダリングエラー、不正なデータ形式など

### 解決方法

1. ページをリロードすると復旧する場合が多い
2. 再現する場合は DevTools のコンソールでエラースタックトレースを確認
3. `app/error.tsx`（グローバル）または `app/chat/error.tsx`（チャット固有）がキャッチしている

---

## 署名 URL の自動リフレッシュが失敗する

### 症状

* 添付画像が壊れたアイコンのまま復旧しない
* 1 回の自動リトライ後も表示されない

### 原因

* Storage バケットのポリシーが未設定
* `app_user` と `auth.uid()` の紐づけが壊れている
* Supabase Storage のサービス障害

### 解決方法

1. Storage ポリシーが正しく設定されているか確認（`docs/database.md` の Storage バケット/ポリシー節参照）
2. ブラウザをリロードすると新しい署名 URL が発行される
3. Supabase Dashboard の Status ページでサービス障害がないか確認
